...
#### HEAP
힙 자체의 자료구조 특성상 비교 함수를 필요로 합니다. 아래의 비교 함수 항목을 참고해 주십시오.

HEAP(T)은 T##_Heap *의 대체된 표현입니다. T 자료형으로 이루어진 힙 노드의 포인터입니다.

NEW_HEAP(T)      T##Heap
PUSH_HEAP(T)     T##PushHeap
POP_HEAP(T)      T##PopHeap
DELETE_HEAP(T)   T##DeleteHeap
IS_EMPTY_HEAP(T) T##IsEmptyHeap
IS_FULL_HEAP(T)  T##IsFullHeap
HEAP_SORT(T)     T##HeapSort

#### LIST

LIST(T)는 T##_List *
ITERATOR(T)        T##_ListIter *
NEW_LIST(T)        T##List()
NEW_ITERATOR(T)    T##ListIter
NEXT(T)            T##Next
HAS_NEXT(T)        T##HasNext
GET(T)             T##Get
APPEND(T)          T##Append
INSERT(T)          T##Insert
REMOVE(T)          T##Remove
DELETE_LIST(T)     T##DeleteList
DELETE_ITERATOR(T) T##DeleteIter
IS_EMPTY_LIST(T)   T##IsEmptyList
IS_FULL_LIST(T)    T##IsFullList
SORT_LIST(T)       T##InsertionSort
...
#### LSDRadixSort
정수 자료형에서만 사용하는 정렬입니다.

## 테스팅 코드
테스트 코드는 python을 활용하여 진행합니다. python에서 자동으로 이미 작성된 코드에 자동으로 문자열을 넣는 식으로 진행됩니다.